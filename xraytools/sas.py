...
def lightcurve(filename, event_list, bins, erange=(0.15,2.4), trange=None, x=None, y=None, r=None, bx=None, by=None, br=None, dump=None):
	"""
	...
	if(xrt._is_subscriptable(erange) and len(erange)==2):
		pass
	else:
	...
	source_filename = f"lightcurves/source_{filename}.lc"
	bkg_filename = f"lightcurves/bkg_{filename}.lc"
	final_filename = f"lightcurves/final_{filename}.lc"
	
	timeselect = ""
	if(xrt._is_subscriptable(trange) and len(trange)%2 == 0):
...
def final_spectrum(filename, countbin):
	"""
	...
	"""
	source = f"spectra/source_{filename}.fits"
	bkg = f"spectra/bkg_{filename}.fits"
	rmf = f"spectra/{filename}.rmf"
	arf = f"spectra/{filename}.arf"
	final = f"spectra/final_{filename}_maxc{countbin}.fits"
...
def spectrum(filename, event_list, countbin, trange=None, x=None, y=None, r=None, bx=None, by=None, br=None, dump=None):
	"""
